...
import { Product, AdjustmentReason, StockCategory, StockLog } from '../types';
import { api } from '../api.ts';

interface InventoryProps {
  products: Product[];
  setProducts: (val: any) => Promise<void>;
  logs: StockLog[];
  setLogs: (val: any) => Promise<void>;
}

const Inventory: React.FC<InventoryProps> = ({ products, setProducts, logs, setLogs }) => {
  ...
  const [adjReason, setAdjReason] = useState<AdjustmentReason>(AdjustmentReason.RESTOCK);
  ...
  const [newProd, setNewProd] = useState({
    ...
    lowStockThreshold: 5,
    gstRate: 18,
    variants: [] as { name: string; priceRetailAdjustment: number; priceWholesaleAdjustment: number; stock: number }[]
  });
  ...
  const updateVariantInForm = (index: number, field: string, value: any) => {
  ...
  const [bulkStatus, setBulkStatus] = useState<{ success?: string; error?: string }>({});
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (productIndex !== -1 && !isNaN(amount)) {
          ...
          newLogs.push({
            ...
            reason: (reason?.trim() as AdjustmentReason) || AdjustmentReason.RESTOCK,
            timestamp: new Date().toLocaleString(),
            user: 'Admin (Bulk)'
          });
          successCount++;
            ...
            <form onSubmit={handleAddProduct} className="p-8 space-y-6">
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="space-y-1.5">
                  ...
                  <select className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-3 font-bold" value={newProd.category} onChange={e => setNewProd({...newProd, category: e.target.value as StockCategory})}>
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md relative z-10 overflow-hidden">
             <div className="p-6 bg-slate-50 border-b flex justify-between"><div><h4 className="font-bold">Adjust Stock</h4><p className="text-xs text-slate-500">{selectedProduct.name}</p></div><button onClick={() => setIsAdjustModalOpen(false)}><X size={20} /></button></div>
              <div className="p-6 space-y-4">
                ...
                <select className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-3 font-bold" value={adjReason} onChange={e => setAdjReason(e.target.value as AdjustmentReason)}>{Object.values(AdjustmentReason).map(r => <option key={r} value={r}>{r}</option>)}</select>

...
const Service = ({ records, setRecords, customers }) => {
  ...
  const amcCustomers = customers.filter(c => c.type === 'AMC');
              ...
              {expiringAMC.map(c => (
                <div key={c.id} className="p-3 bg-amber-50 rounded-xl border border-amber-100">
                  <div className="flex justify-between items-start mb-1">
                    <p className="font-bold text-sm text-amber-900">{c.name}</p>
                    <span className="text-[10px] font-black text-amber-600 uppercase">
                      Expiring
                      </span>
                      </div>
                  <p className="text-xs text-amber-700">
                    Expiry: {c.amcExpiry}</p>
                </div>
              ))}

              {expiringAMC.length === 0 && <p className="text-center text-slate-400 text-xs py-4">
                All AMCs healthy.</p>}
            </div>
          </div>
        </div>
            ...
            <form onSubmit={handleLogService} className="p-8 space-y-5">
              <div className="space-y-1.5 relative">
                ...
                <div className="relative">
                   ...
                   <input required 
                   className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl pl-10 pr-4 py-3 font-bold"
                   ...
                   onClick={() => setSelectedCustomer(null)}
                    className="absolute right-3 top-3.5 text-slate-400 hover:text-red-500"><X size={18} /></button>)}
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5"><label className="text-[10px] font-black uppercase text-slate-400 ml-1">Visit Type</label><select className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-3 font-bold" value={newRecord.type} onChange={e => setNewRecord({...newRecord, type: e.target.value })}><option value="SERVICE">Regular Service</option><option value="INSTALLATION">Installation</option><option value="REPAIR">Repair/Complaints</option></select></div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5"><label className="text-[10px] font-black uppercase text-slate-400 ml-1">Status</label><select className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-3 font-bold" value={newRecord.status} onChange={e => setNewRecord({...newRecord, status: e.target.value })}><option value="COMPLETED">Completed</option><option value="SCHEDULED">Scheduled</option><option value="PENDING">Pending Part</option></select></div>
              </div>

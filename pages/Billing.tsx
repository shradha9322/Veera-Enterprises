...
import { Product, InvoiceItem, PaymentMode, Customer, Invoice } from '../types';

interface BillingProps {
  products: Product[];
  customers: Customer[];
  onBillFinalized: (invoice: Invoice) => void;
}

const Billing: React.FC<BillingProps> = ({ products, customers, onBillFinalized }) => {
      ...
      <div className="flex flex-col gap-6">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 sticky top-6">
          ...
          <div className="space-y-4 mb-8">
            ...
            <div className="flex justify-between items-center text-slate-500 font-medium">
              ...
              <div className="relative w-28">
                ...
                <input 
                  type="number"
                  className="w-full pl-5 pr-2 py-1.5 bg-slate-50 border-none rounded-lg text-right font-black text-blue-600 text-sm"
                  value={discount}
            ...
            <div className="flex justify-between items-end">
              ...
              <span className="font-black text-3xl text-blue-600">₹{grandTotal.toFixed(0)}</span>
            </div>
          </div>
          ...
          <button 
            ...
            className="w-full bg-slate-900 text-white py-5 rounded-2xl font-black text-lg shadow-2xl hover:bg-black transition-all active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed mb-4"
          >
            Finalize & Save Bill
          </button>
          
          <div className="grid grid-cols-2 gap-3">
             <button onClick={() => window.print()} className="bg-slate-100 text-slate-700 py-3 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-slate-200 transition-all border border-slate-200">
               <Printer size={18} /> Print
             </button>
             <button onClick={shareOnWhatsApp} className="bg-emerald-50 text-emerald-600 py-3 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-emerald-100 transition-all border border-emerald-100">
        ...
        <div className="bg-blue-600 text-white p-6 rounded-3xl shadow-xl relative overflow-hidden">
          ...
          <button className="text-[10px] font-black uppercase tracking-widest text-blue-200 border-b border-blue-400 pb-1">Read Documentation</button>
        </div>
      </div>
